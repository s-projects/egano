...
#ifdef _WIN32
const std::string DAEMON_FILENAME = "eganod.exe";
#else
const std::string DAEMON_FILENAME = "eganod";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_eganod_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"eganod" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), "eganod", "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
