...
const command_line::arg_descriptor<uint16_t> arg_daemon_port = { "daemon-port", "Use daemon instance at port <arg> instead of 47867", 0 };
...
simple_wallet::simple_wallet(System::Dispatcher& dispatcher, const CryptoNote::Currency& currency, Logging::LoggerManager& log) :
  ...
  m_consoleHandler.setHandler("export_keys", boost::bind(&simple_wallet::export_keys, this, _1), "Show secret keys of the wallet");
...
bool simple_wallet::init(const boost::program_options::variables_map& vm) {
  ...
  if (m_generate_new.empty() && m_wallet_file_arg.empty()) {
    ...
    std::cout << "[O]pen existing wallet, [G]enerate new wallet file, [I]mport keys or [E]xit.\n";
    char c;
    do {
      std::string answer;
      std::getline(std::cin, answer);
      c = answer[0];
      if (!(c == 'O' || c == 'G' || c == 'E' || c == 'o' || c == 'g' || c == 'e' || c == 'I' || c == 'i')) {
